...
class BotController:
    ...
    def update(self, dt: float, host_entity: BotPlayerInfo, world: WorldInfo) -> None:
        ...
        timer_trigger = self._memory.tick_reeval(dt)
        is_in_danger = self._memory.config.danger_check_type.is_in_danger(
            world, host_entity, self._memory.config.danger_radius
        )
        ...
        if self._current_state:
            next_state = self._current_state.on_tick(
    ...
    def _perform_global_reevaluation(self, world: WorldInfo, bot: BotPlayerInfo):
        ...
        # 1 - Danger
        if self._memory.config.danger_check_type.is_in_danger(world, bot, self._memory.config.danger_radius):
            if not isinstance(self._current_state, EscapeState):
        ...
        if random.random() <= config.powerup_chance:
            ...
            path = config.powerup_policy.get_path(world, bot, self._memory)
            if target and path:
...
class BotFactory:
    ...
    def make(cls, bot_type: BotType) -> BotControllerInfo:
        ...
        _memory = BotMemory(config)
        return BotController(_memory)
