...
from common_types import  Action, ActionInfo, BotConfigInfo, BotPlayerInfo, BotState, PlayerAction, BotMemoryInfo, EntityType, GridCoords, WorldInfo
...
class WanderState:
    def on_enter(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> None:
    ...
    def on_tick(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> BotState | None:
    ...
    def decide_action(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> ActionInfo | None:
...
class EscapeState:
    def on_enter(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> None:
        ...
        dangerous_cells = config.danger_check_type.get_all_danger_zones(
            world)
    ...
    def on_tick(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> BotState | None:
        ...
        danger_zones = config.danger_check_type.get_all_danger_zones(world)

        if not memory.path:
            self.on_enter(config, memory, world, entity)
...
class GetPowerupState:
    ...
    def on_enter(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> None:
        ...
        path = config.powerup_policy.get_path(world, entity, memory)
        memory.set_path(path)
    ...
    def decide_action(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> ActionInfo | None:
        allow_bomb = config.powerup_policy.can_place_bomb()
...
class AttackState:
    ...
    def on_enter(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> None:
        memory.set_goal(self._target_goal)
        path = config.attack_policy.get_path(world, entity, memory)
        memory.set_path(path)
    ...
    def decide_action(self, config: BotConfigInfo, memory: BotMemoryInfo, world: WorldInfo, entity: BotPlayerInfo) -> ActionInfo | None:
        ...
        if memory.goal:
            ...
            if dist <= config.attack_range_trigger:
